# Perry Paper Trading Laboratory Report

## Experiment Summary

- Experiment Duration: 1 cycle(s)
- Asset: BTCUSDT
- Starting Capital: 100000.00
- Ending Capital: 100000.00
- Net Return: 0.00%
- Prediction Count: 1
- Trade Count: 0
- Trades Skipped: 1
- Win Rate: 0.00%
- Profit Factor: 0.00
- Average Trade: 0.00
- Largest Win: 0.00
- Largest Loss: 0.00
- Maximum Drawdown: 0.00%

## Confidence Calibration

- 80-90%: predictions=1, trades=0, hit_rate=0.00%

## Performance By State

- Directional Expansion: predictions=1, trades=0, win_rate=0.00%, profit_factor=0.00

## Driver Observations

- Above equilibrium: 1
- Structure support intact: 1
- Resistance overhead: 1
- Positive trend geometry: 1

## Common Entry Reasons

- Directional Expansion state|high confidence|high confluence|27% move probability|upper structure|evidence below execution threshold: 1

## Common Exit Reasons

- None recorded.

## Objective Observations

- Perry generated observations, but the decision layer found no setup with enough independent evidence.
- State-level statistics remain sample-limited for: Directional Expansion.

## Evidence-Based Recommendations

- Continue collecting observations before changing predictive logic.
- Compare confidence buckets only after each bucket has a meaningful sample size.
- Investigate states with frequent no-trade outcomes separately from losing traded states.
